...
#Read in trap file
traps<-read.table("C:/Users/jnelson/Documents/OSU/ElkSCR/McKenzie/2019McKTraps.txt")
head(traps)
colnames(traps)<-c("TransectID", "X", "Y", "sep", "Effort", "Julian", "Precip", "Observers")
head(traps)

#Read in coordinates and sex of identified elk
McKElk2019<-read.csv("C:/Users/jnelson/Documents/OSU/ElkSCR/McKenzie/2019McKElkDDSex.csv", header=TRUE)
head(McKElk2019)
...
McKElk2019$CollectionDate<-as.Date(McKElk2019$CollectionDate, "%m/%d/%y")

require(lubridate)
McKElk2019$CollectionDate = yday(McKElk2019$CollectionDate)
head(McKElk2019)

McKElk2019$CollectionDate<-McKElk2019$CollectionDate - 156
head(McKElk2019)
...
head(McKElk2019)
McKElk2019<-McKElk2019[,c(8,3,9,4,5,7)]
head(McKElk2019)
...
write.csv(McKElk2019.UTM, "2019McKElk.csv", row.names=FALSE, quote = FALSE)

#Calculate distances between individuals and nearest trap
library(sf)
a<-st_as_sf(McKElk2019.UTM, coords=c("X", "Y"), crs="+proj=utm +zone=10 +datum=NAD83 
            +units=m +no_defs")
